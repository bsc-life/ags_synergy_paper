...
def get_experiment_colors(exp_name):
    ...
    if 'control' in exp_type:
        return color_scheme['control']
    elif 'pi3k' in exp_type and 'mek' in exp_type:
        return color_scheme['pi3k_mek']
    elif 'akt' in exp_type and 'mek' in exp_type:
...
def process_instance_top_10(row, experiment_folder, sim_wt_df, experimental_data_path, drug_name):
...
def process_top_10(csv_path, experiment_name, experimental_data_path, drug_name, top_n, nodrug=False):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
        futures = [executor.submit(process_instance_top_10, row, experiment_folder, sim_wt_df, experimental_data_path, drug_name) for _, row in top_10_df.iterrows()]

        for future in concurrent.futures.as_completed(futures):
            result = future.result()
            if result is not None:
                all_sim_growths.append(result)
    ...
    print(f"Processing experimental data for {drug_name}")
    ...
    print(f"Calculating correlation for {drug_name}")
    y_sim = sim_stats["mean_alive"][:len(y_exp)]
    pearson_corr, p_value = pearsonr(y_sim, y_exp)

    # Save correlation results
    print(f"Saving correlation results for {drug_name}")
    ...
    print(f"Plotting with Cell Systems aesthetics for {drug_name}")
    if nodrug:
    ...
    print(f"Top N plot saved to {plot_path_png}")
...
def process_last_gen(experiment_name, experimental_data_path, drug_name):
    ...
    plt.savefig(plot_path_png, dpi=300, bbox_inches='tight', transparent=True)
    print(f"Last generation plot saved to {plot_path_png}")
    # plt.savefig(plot_path_svg, bbox_inches='tight', transparent=True)
...
def combine_and_plot_experiments(experiment_info, output_path):
    """
    Create a plot combining growth curves from multiple experiments.
    """
    ...
    for idx, exp in enumerate(experiment_info):
        ...
        colors = get_experiment_colors(exp["name"])
...
def side_by_side_experiment_plots(experiment_info, output_path):
    ...
    for idx, exp in enumerate(experiment_info):
        ...
        # Experimental data (left subplot)
...
best_pi3kmek_combined_experiment_name = "synergy_sweep-pi3k_mek-0804-0922-18p_transient_delayed_uniform_10k"
best_pi3kmek_PI3K_singledrug_experiment_name = "synergy_sweep-pi3k_mek-0904-0929-18p_PI3K_transient_delayed_uniform_10k_singledrug"
best_pi3kmek_MEK_singledrug_experiment_name = "synergy_sweep-pi3k_mek-0904-0929-18p_MEK_transient_delayed_uniform_10k_singledrug"

best_aktmek_combined_experiment_name = "synergy_sweep-akt_mek-0804-1856-18p_transient_delayed_uniform_10k"
best_aktmek_AKT_singledrug_experiment_name = "synergy_sweep-akt_mek-0904-0929-18p_AKT_transient_delayed_uniform_10k_singledrug"
best_aktmek_MEK_singledrug_experiment_name = "synergy_sweep-akt_mek-0904-0929-18p_MEK_transient_delayed_uniform_10k_singledrug"
...
pi3k_mek_synergy_experiments = [
    {
        ...
        "strategy": get_strategy_from_filename(best_control_experiment_name),
        "drug_name": get_drug_from_filename(best_control_experiment_name),
        "short_name": "CTRL_CMA_top10_linear"
    },
    {
        ...
        "short_name": "PI3K_sweep_top10_linear"
    },
    {
        ...
        "short_name": "MEK_sweep_top10_linear"
    },
    {
        ...
        "short_name": "PI3KMEK_top10_linear"
    }
]
...
for experiment in pi3k_mek_synergy_experiments:
    ...
    if "singledrug" not in experiment["name"]:
        # Call process_top_10 for each experiment
        exp_data = process_top_10(csv_path, experiment["name"], experimental_data_path, experiment["drug_name"], experiment["top_n"])
...
# Example of using logging in the main script
