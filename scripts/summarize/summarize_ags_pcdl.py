...
def process_and_plot_nodes(df, instance_folder, ax=None):
    ...
    # Select columns containing 'node' but not 'anti', 'time' or 'drug_X'
    node_cols = df.filter(regex='node').filter(regex='^(?!.*anti|time|drug_X)').columns
...
def save_pcdl_timeseries(output_folder, instance_folder):
    ...
    mcdsts = pcdl.pyMCDSts(output_path = output_folder, graph=False, verbose=False)
