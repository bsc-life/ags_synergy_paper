...
PROGRAM_NAME := physiboss-drugs-synergy-model

# CC := g++ # default compilation
CC := g++ -g # Add "-g" for debugging
...
CUR_DIR = $(shell pwd)
CUSTOM_DIR = sample_projects_intracellular/ags_model/custom_modules
...
CFLAGS := -g -march=$(ARCH)  -Og -ggdb -fomit-frame-pointer -mfpmath=both -fopenmp -m64 -std=c++11 
...
# Specific to AGS
PhysiCell_custom_module_OBJECTS := custom.o submodel_data_structures.o drug_transport_model.o boolean_model_interface.o
...
drug_transport_model.o: ./custom_modules/drug_transport_model.cpp
	$(COMPILE_COMMAND) $(INC) -c ./custom_modules/drug_transport_model.cpp
...
PROJ := physiboss-drugs-synergy-model
