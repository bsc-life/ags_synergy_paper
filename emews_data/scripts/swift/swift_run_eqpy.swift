...
(string result) run_obj(string custom_parameters, int algo_iteration, int parameter_iteration, int num_replications, string executable, string default_settings)
{
    ...
    // file summarize_py = input(emews_root + "/scripts/summarize/summarize_ags_pcdl_nord4.py");
    // file summarize_py = input(emews_root + "/scripts/summarize/summarize_ags_pcdl.py");
    // USING THE RES ONE ONLY SAVES THE CSV FILE
    file summarize_py = input(emews_root + "/scripts/summarize/summarize_ags_pcdl_res.py");    
    ...
    foreach replication in [0:num_replications-1:1] {
      ...
      make_dir(instance_dir) => {
        ...
        (out,err) = run_model(model_sh, executable, instance_settings, instance_dir) => {
          ...
          results_array[replication] = get_result(drug_name, metric, instance_dir, exp_path, custom_parameters) =>
          results2json(custom_parameters, instance_dir) =>
          rm_dir(instance_dir + "output/") =>
          rm_dir(instance_dir + "simulation_growth.csv");
