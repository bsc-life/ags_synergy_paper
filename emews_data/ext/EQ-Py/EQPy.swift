...
(void v) _void_py(string code, string expr="\"\"") "turbine" "1.2.3"
    [ "turbine::python 1 1 <<code>> <<expr>> "];

@dispatch=resident_work
(string output) _string_py(string code, string expr) "turbine" "1.2.3"
...
(void v) EQPy_put(location loc, string data){
    printf("EQPy_put called with: %s", data);
    string code = put_string % data;
    printf("EQPy_put code: %s", code);
    @location=loc _void_py(code) => v = propagate();
}
